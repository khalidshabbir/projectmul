...
const StudentsTimetable = () => {
    ...
    const [getClassTimetable ,setClassTimetable] = useState("")
    useEffect(() => {
    ...
    const handlesearch = async () => {
        try {
            const response = await axios.get(base_url + `/get_timetable_students/${getdayvalue}/${getclassvalue}`);
    ...
    const handlechangeStudent=async(e)=>{
        ...
        try {
            ...
            const response = await axios.get(base_url + `/get_timetable_studentstimetable/${getclassvalue}`);
    ...
    return (
        <>
            <Navbar />
            <Hero img={img} title={"Student Timetable"} />
            <section className='container'>
                <div className='mx-lg-5 p-5 '>
                    <h4 className='text-center mb-5'>Student Timetable</h4>
                   ...
                   { gettimetable?
                   <div className='mt-3'>
                    { getclassvalue?<div className='btn-wrapper text-end me-5'><button className='btn btn-primary shadow-none' onClick={ handlechangeStudent}>Print</button></div>:<div></div>}
                    ...
                    <div className='d-flex algin-items-center justify-content-center mt-5'><BsInbox className='text-secondary' style={{fontSize:"150px"}}/></div>}
                </div>
            </section>
            <Footer />
        </>
