...
const Rooms = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(base_url + '/register_classrooms',
  ...
  const handleDelete = async (classroomId) => {
    try {
      ...
      const response = await axios.delete(base_url+`/delete_classrooms/${classroomId}`);
  ...
  const updateFunction= async(id)=>{
    try {
      const response = await axios.put(base_url+`/update_classrooms/${id}`, { name:updateClassroomvalue });
  ...
  useEffect(() => {
    const fetchClassrooms = async () => {
      try {
        const response = await axios.get(base_url + '/get_classrooms');
                  ...
                  <>
                    <tr key={classroom._id}>
                      ...
                      <td className='text-end'>
                        <button className='btn btn-dark shadow-none' onClick={()=>{setupdateClassroom(classroom)
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form>
              <div className='p-3'>
                <div class="mb-3">
                  <label for="forclassrooms" class="form-label">Enter ClassRoom Name</label>
                  <input type="text" required value={classroom}
                    onChange={(e) => setClassRoom(e.target.value)} class="form-control shadow-none" placeholder='Information Security Room# 406' id="forclassrooms" aria-describedby="emailHelp" />
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form>
              <div className='p-3'>
                <div class="mb-3">
                  <label for="forclassrooms" class="form-label">Edit ClassRoom Name</label>
