...
const Teachers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(base_url + '/register_teacher',
  ...
  const handleDelete = async (Id) => {
    try {
      ...
      const response = await axios.delete(base_url + `/delete_teachers/${Id}`);
  ...
  const updateFunction = async (id) => {
    try {
      const response = await axios.put(base_url + `/update_teachers/${id}`, { name: updateClassroomvalue });
    ...
    <>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form>
              <div className='p-3'>
                <div class="mb-3">
                  <label for="forclassrooms" class="form-label">Enter Teacher Name</label>
                  <input type="text" required value={classroom}
                    onChange={(e) => setClassRoom(e.target.value)} class="form-control shadow-none" placeholder='Dr.Tariq' id="forclassrooms" aria-describedby="emailHelp" />
                </div>
      ...
      <div class="modal fade" id="updateclassrooms" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header border-none ">
              <h5>Update Teacher</h5>
              <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form>
              <div className='p-3'>
                <div class="mb-3">
                  <label for="forclassrooms" class="form-label">Edit Teacher Name</label>
